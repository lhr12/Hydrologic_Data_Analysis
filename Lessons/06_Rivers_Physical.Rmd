...
#discharge = 1332 + 1725*log(R)
Eno.RImodel$coefficients[1] + Eno.RImodel$coefficients[2]*log(500)
#assuming stationarity: the future is going to behave like the past
```
...
>The predicted 100 and 500 year events were smaller using the 30 year historical data versus the full historical dataset. I would expect the the recurrence interval discharge expectations to be at least the same or even larger than predicted by the most recent models.  
...
> Use the most up to date data, or even base 30-year models on the most recent 30 years.
...
SaladoRecurrenceLate <-  SaladoDischarge %>%
  group_by(Year) %>%
  filter(Year > 1985) %>%
  summarise(PeakDischarge = max(Discharge)) %>% 
  mutate(Rank = rank(-PeakDischarge), 
...
SaladoRecurrencePlot.Late <- 
  ggplot(SaladoRecurrence, aes(x = RecurrenceInterval, y = PeakDischarge)) +
  geom_point() +
  geom_point(data = SaladoRecurrenceLate, color = "#02818a",
             aes(x = RecurrenceInterval, y = PeakDischarge))
print(SaladoRecurrencePlot.Late)

Salado.RImodel.Late <- lm(data = SaladoRecurrenceLate, PeakDischarge ~ log(RecurrenceInterval))
summary(Salado.RImodel.Late)

Salado.RImodel.Late$coefficients[1] + Salado.RImodel.Late$coefficients[2]*log(100)
Salado.RImodel.Late$coefficients[1] + Salado.RImodel.Late$coefficients[2]*log(500)
