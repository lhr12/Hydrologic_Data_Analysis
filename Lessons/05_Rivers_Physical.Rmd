...
-flow velocity (horizontal distance/time)
-slope
-inputs
-stage
-roughness -> erosion
-cross-sectional area
-mixing and temperature (gradient?)
...
print(DvsGHplot)
#curve is logarithmic. when you flip the axes, curve becomes exponential
```
...
>Wetter regions have higher discharge (NC, Montana), the Nehalem and Sauk have similar scales, and the Verde has the least discharge with random anomaly spikes.
...
>In areas where precipitation is less seasonal, like the Eno and the Sauk, there do not seem to be annual patterns of discharge, but just more anomalies spread out irregularly across the graph. HOwever, the Sauk river is in a cold enough climate that precipitation falls as snow and flows may be frozen, so discharge decreases in winter months. This seasonal cycle seems to have broken down over the last 5 years or so, this could possibly be attributed to a warming climate. The Nehalem River seems to have seasonal runoff in winter months, the Bitterroot seems to have seasonal runoff in the spring (perhaps due to snowmelt).  The Verde river does nothave any seasonality since it is not constantly flowing, but rather has rare jumps in runoff from specific rainstorm events.
...
>Make the verde y axis on a log scale to better be able to visualize anomalies; Make a column with site name to be able to facet graph. Can do it in one plot rather than combining 5 different plots.

***RBIND!!! TO COMBINE DATASETS ONE ON TOP OF THE OTHER!!!!!!***
...
BitterrootDischarge.Historical <- mutate(BitterrootDischarge.Historical, 
                                         DOY = yday(Date))
#yday in lubridate package = day of year
