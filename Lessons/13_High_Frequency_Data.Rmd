...
>precipitation, respiration, photosynthesis, contaminant transport, temperature, dissolved oxygen, nutrient inputs/outputs, 
...
>overland flow, groundwater, rain directly in the river channel, pipes from sewers. Storms create peaks in graph (quickflow), baseflow is flow of river after rain event.
...
> 51.9 % baseflow and 48.1% quickflow
...
> 43.4% baseflow and 56.6% quickflow

What does this mean physically about the watershed?

> There are more impermeable surfaces with less opportunities for infiltration into soils and groundwater, less plants to take up excess runoff than Third Fork Creek.
...
> high quickflow percentages, low baseflow percentages; a lot of water coming out of pipes going directly into streams, eroded stream beds that are deeper and more incized, water quality parameters that match these conditions
...
>Ellerbe Creek is flashier than Third Fork Creek.

```{r}
#Doing it on our own for a different site
dischargedatVR2018 <- readNWISuv(
  site = "03550000", #VALLEY RIVER AT TOMOTLA
  parameterCd = "00060",
  startDate = "2018-01-01", 
  endDate = "2019-01-01"
  ) %>% 
...
VRbaseflow <- BaseflowSeparation(
  dischargedatVR2018$Flow_Inst, 
  filter_parameter = 0.925, 
  passes = 3
  )

VR2018 <- cbind(dischargedatVR2018, VRbaseflow)
...
VRsitedata <- readNWISsite(site = "03550000")
VR.catchment.size <- VRsitedata$drain_area_va #square miles

RBI_VR2018 <- (
  sum(abs(diff(dischargedatVR2018$Flow_Inst))) / 
    sum(dischargedatVR2018$Flow_Inst[-1])
  ) / VR.catchment.size
