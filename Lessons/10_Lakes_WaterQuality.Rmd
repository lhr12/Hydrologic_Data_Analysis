...
> Variable 1: lulc 500 buffer 2011 21 pct

> Variable 2: lulc 500 buffer 2011 23 pct

> Variable 3: atm.dep. of N
...
LAGOSlucl <- LAGOSdata$buffer500m.lulc
LAGOSchag <- LAGOSdata$hu8.chag

LAGOStotal <- left_join(LAGOSlocus, LAGOSstate, by = "state_zoneid") %>%
  left_join(LAGOSnutrient, LAGOStotal, by = "lagoslakeid") %>%
  left_join(LAGOSlucl, LAGOStotal, by = "lagoslakeid") %>%
  left_join(LAGOSchag, LAGOStotal, by = "hu8_zoneid") %>%
  select(lagoslakeid, sampledate, chla, tp, tn, secchi, 
         gnis_name, lake_area_ha, state, state_name, 
         buffer500m_nlcd2011_pct_21, buffer500m_nlcd2011_pct_23, 
         hu8_dep_totaln_2010_mean) %>%
  drop_na()

names(LAGOStotal)[11:13] <- c("Crop", "CAFO", "DepN")
```
...
```{r}
corrplot(cor(LAGOStotal[,c(3:6,11:13)]), method = "circle")
```
...
```{r}
tn.test <- lm(tn ~ chla + secchi + buffer500m_nlcd2011_pct_21 + 
   buffer500m_nlcd2011_pct_23 + hu8_dep_totaln_2010_mean, LAGOStotal)
summary(tn.test)

tp.test <- lm(tp ~ chla + secchi + buffer500m_nlcd2011_pct_21 + 
   buffer500m_nlcd2011_pct_23 + hu8_dep_totaln_2010_mean, LAGOStotal)
summary(tp.test)
...
> For tn it accounts for 18% of the variance for nitrogen, for tp it accounts for 46 % of the variance
