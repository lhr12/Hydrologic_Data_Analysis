...
-physical features of lakes, rivers, watersheds
-visualizing results
-able to see spatial distributions
-may be helpful in coming up with research questions that would have otherwise not have been obvious
-where contaminants are originating from 
-
...
* **Point** <no dimension or area, holds information about location w/ lat & long>
* **Line** <connection between 2+ points, 1-2 dimensions>
* **Polygon** <have area, 2 dimensions>
...
secchi.summary <- LAGOScombined %>%
  ...
  summarise(secchi.mean = mean(secchi),
            ...
            depth = mean(maxdepth),
            lat = mean(nhd_lat), 
            long = mean(nhd_long)) %>% #need to tell it what to populate with, have to include lat long depth area etc in order for them to be in the df
  drop_na()
```
