...
> aggregation: combining data points into summarized groups (mean, median, range, etc). could be used to eliminate seasonality and look at larger trends. ex: daily -> weekly or monthly data. used to reduce number of rows in dataset.

> subsampling: using only part of dataset, can zoom in on particular time period or even out frequency of data samples

> interpolation: using existing data to generate data to fill in gaps
...
* **Piecewise constant**: <at any point, you're going to take the nearest neighbor and use that as your estimation>
* **Linear**: <connect data points with a line, any point predicted to fall on that line between points>
* **Spline**: <uses quadratic formulas to interpolate data, might be able to catch unknown local max and mins>
...
linearinterpolation <- as.data.frame(approx(Mendotadata, n = 303, method = "linear")) #generates dates as days since 1/1/1970
...
#z-score tells you direction of the trend (posidive or negative) and the magnitude
#S = difference between one point and the next summed up. Larger S = greater magnitude of trend 
#varS = 
```

What would we conclude based on these findings? Describe as you would to an educated but non-specialist audience.

>cannot reject null hypothesis, no significant trend over time. if it was significant, would be negative trend. there is a decreasing trend in phosphorus for the months of October and November between 1988 and 2013.
...
ClearCreekDischarge.Monthly <- ClearCreekDischarge %>%
  mutate(Year = year(Date), 
         Month = month(Date)) %>%
  group_by(Year, Month) %>% #want monthly summaries for each year, so also group by year
