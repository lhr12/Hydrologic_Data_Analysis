...
options(scipen = 100) #keep number out of scientific notation up to 100th decimal place
...
#left join means that we take everything from df on the left and add columns from df on the right by the column that they both have in common
...
ggplot(LAGOStrophic, 
  ...
  geom_jitter(alpha = 0.2) + #within a month, it will spread out the data points randomly so you can visualize the data better
...
> ChlA predicts more eutrophic and hypereutrophic lakes than secchi depth and phosphorus
